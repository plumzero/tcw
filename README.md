...
- 事件反应
    ...
    + 在这一层执行 fork 子进程和子进程重新调起，并对进程信号进行处理。
- 模块回调
    + 模块回调采用同步回调处理方式；
    + 所负责的具体业务主要包括 消息校验、心跳、定时器等；
    + 具体见功能说明；
- 函数回调
    + 可以在这里定义进程杀灭，以及来自其他地方的消息的处理，同时可以定义服务。
...
- 信号处理
    ...
    + 对于具体的服务线程，后续可以对一些信号，如 SIGSEGV, SIGBUS 等进行屏蔽处理，但应该没
      有必要；
- fork 子进程和子进程重新调起
...
- 进程服务
    + 进程服务通过 id 进行标识，该id取自服务名称(从配置中读取)sha1计算后前8个字节转换后的无
      符号整型。
    + 关于 eeh 中对进程服务id的定义。暂略。
